...
export async function middleware(req: NextRequest) {
  ...
  if (!token) return NextResponse.next(); // let /chat render

  try {
    const { payload } = await jwtVerify(token, jwks, { algorithms: ['RS256'] });
    const session = {
      tid: (payload as any).TenantId || (payload as any)?.custom?.tenant_id || null,
      uid: payload.sub, email: (payload as any).email, role: (payload as any).role || 'member'
    };
    ...
    return NextResponse.next(); // invalid token → still render /chat
  }
}
